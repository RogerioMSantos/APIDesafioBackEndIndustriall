...
public class EventController(EventService eventService
    , UserService userService, EventUserService eventUserService) : ControllerBase
{
    [HttpGet]
    public async Task<IActionResult> GetEvents()
    {
        ...
        foreach (var iEvent in lEvent) iEvent.Participants = GetParticipants(iEvent);
        return Ok(lEvent);
    }

    private List<User> GetParticipants(Event @event)
    {
        return eventUserService.GetUsers(@event);
    }
    ...
    public async Task<IActionResult> UpdateEvent(int id, [FromBody] Event uEvent)
    {
        ...
        if (oldEvent is null) return NotFound();
    ...
    public async Task<IActionResult> DeleteEvent(int id)
    {
        ...
        if (fEvent is null) return NotFound();
